<?php
class User_m extends MY_Model{

	protected $table = 'users';
	protected $table_2 = 'events';
	protected $table_3 = 'events_staff';
	protected $table_4 = 'events_package_staff';

	// public function get_events($id=false){
	// 	$id = !$id ? get_session('id') : $id;

	// 	$this->db->distinct();
	// 	$this->db->select('e.id, e.*');
	// 	$this->db->from($this->table . ' as u');
	// 	$this->db->join($this->table_3 . ' as es', 'u.id = es.user_id', 'left');
	// 	$this->db->join($this->table_4 . ' as p', 'u.id = p.user_id', 'left');
	// 	$this->db->join($this->table_2 . ' as e', 'e.id = es.event_id or e.id = p.event_id');
	// 	$this->db->where('u.id',$id);

	// 	if('id' == $this->data['order_by']){
	// 		$order_by = 'e.id';
	// 	}else{
	// 		$order_by = $this->data['order_by'];
	// 	}
	// 	$this->db->order_by( $order_by . ' ' . $this->data['order'] );

	// 	$query = $this->db->get();

	// 	if( $query ){
	// 		return $query->result();
	// 	}

	// 	return false;
	// }
	public function get_events($user_id=false){

		$user_id = !$user_id ? get_session('id') : $user_id;

		$this->db->select("e.*, 'Event' t");
		$this->db->from($this->table . ' as u');
		$this->db->join($this->table_3 . ' as es', "es.user_id = u.id");
		$this->db->join($this->table_2 . ' as e', "e.id = es.event_id");
		$this->db->where('u.id', $user_id);

		$query1 = $this->db->get_compiled_select();
		$this->db->select("e.*, 'Packaging' t");
		$this->db->from($this->table . ' as u');
		$this->db->join($this->table_4 . ' as eps', "eps.user_id = u.id");
		$this->db->join($this->table_2 . ' as e', "e.id = eps.event_id");
		$this->db->where('u.id', $user_id);

		$query2 = $this->db->get_compiled_select();
		$query = $this->db->query("SELECT *, GROUP_CONCAT(t) event_type FROM (" . $query1." UNION ".$query2.") as temp GROUP BY id");

		if( $query ){
			return $query->result();
		}

		return false;
	}

	public function get_by_ids($ids){

		$this->db->select( '*', false );
		$this->db->from( $this->table, false );
		$this->db->where_in('id', $ids);
		$query = $this->db->get();
	    if( $query ){
	    	return $query->result();
	    }

	    return false;
	}

}